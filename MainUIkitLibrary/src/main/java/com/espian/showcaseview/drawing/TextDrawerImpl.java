...
public class TextDrawerImpl implements TextDrawer {
    ...
    @Override
    public void draw(Canvas canvas, float baseY) {
        if (!TextUtils.isEmpty(mDetails) && mDetailSpan != null) {
            Paint mPaint = new Paint();
            mPaint.setColor(mDetailSpan.getTextColor().getDefaultColor());
            mPaint.setTextSize(mDetailSpan.getTextSize());
            mPaint.setAntiAlias(true);
            Rect bounds = new Rect();
            mPaint.getTextBounds(mDetails.toString(), 0, mDetails.length(), bounds);
            canvas.drawText(mDetails.toString(), (canvas.getWidth() - bounds.width()) / 2, baseY - bounds.height(),
                    mPaint);
            baseY = baseY - bounds.height() - offset;
        }
        if (!TextUtils.isEmpty(mTitle) && mTitleSpan != null) {
            Paint mPaintTitle = new Paint();
            mPaintTitle.setColor(mTitleSpan.getTextColor().getDefaultColor());
            mPaintTitle.setTextSize(mTitleSpan.getTextSize());
            mPaintTitle.setAntiAlias(true);
            Rect bounds = new Rect();
            mPaintTitle.getTextBounds(mTitle.toString(), 0, mTitle.length(), bounds);
            canvas.drawText(mTitle.toString(), (canvas.getWidth() - bounds.width()) / 2, baseY - bounds.height(),
                    mPaintTitle);
        }
    }
