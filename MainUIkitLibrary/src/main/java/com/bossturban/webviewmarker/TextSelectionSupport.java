...
public class TextSelectionSupport implements TextSelectionControlListener, OnTouchListener, OnLongClickListener, DragListener,LongPressModeActionListener {
    ...
    public static enum StartTouchType{
        TOUCH_TAP,//tap
        TOUCH_LONG, //hold
        TOUCH_MAGNIFIER //Magnifier
    }
    ...
    public static StartTouchType mCurrentType = StartTouchType.TOUCH_TAP;
    ...
    private TextSelectionSupport(Activity activity, WebView webview,GestureDetector gust) {
        mActivity = activity;
        mWebView = webview;
        mGust = gust;
    }
    ...
    public static TextSelectionSupport support(Activity activity, WebView webview,GestureDetector gust) {
        final TextSelectionSupport selectionSupport = new TextSelectionSupport(activity, webview,gust);
        selectionSupport.setup();
        return selectionSupport;
    }
    public  void setTouchModeType(StartTouchType type) {
        mCurrentType = type;
    }

    public LongPressModeActionListener setLongModeActionListener() {
         return this;
    }
    ...
    public void selectionIgnore() {
        Log.e("TAG", "selectionIgnore");
        if (mSelectionListener != null) {
            mSelectionListener.ignoreSelection();
        }
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        final Context ctx = mActivity;
        if (mCurrentType != StartTouchType.TOUCH_MAGNIFIER) {
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                        if (mCurrentType == StartTouchType.TOUCH_TAP) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (Build.VERSION.SDK_INT>=19) {
                        ...
                        if (mCurrentType != StartTouchType.TOUCH_LONG){
                            return false;
                        }else {
                            if (mKitkat)
                            return true;
        ...
        if (mGust != null) {
            return mGust.onTouchEvent(event);
        }else {
            return false;
        }
    }
    @Override
    public void shouldStartPosition(MotionEvent event) {
        mKitkat = true;//android 4.4 override action mode.
        final Context ctx = mActivity;
        float xPoint = getDensityIndependentValue(event.getX(), ctx) / getDensityIndependentValue(mScale, ctx);
        float yPoint = getDensityIndependentValue(event.getY(), ctx) / getDensityIndependentValue(mScale, ctx);
        final String startTouchUrl = String.format("javascript:android.selection.startTouch(%f, %f);", xPoint, yPoint);
